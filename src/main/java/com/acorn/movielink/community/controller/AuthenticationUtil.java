...
//
//            // memId 조회 추가 (이메일 기반)
//            Optional<Member> memberOpt = memberService.findByEmail(userDetails.getUsername());
//            Integer memId = memberOpt.map(Member::getMemId).orElse(null);
...
//            UserDetails userDetails = (UserDetails) principal;
//
//            // MemberService를 통해 memId 조회
//            Optional<Member> memberOpt = memberService.findByEmail(userDetails.getUsername());
//            return memberOpt.map(Member::getMemId)
//                    .orElseThrow(() -> new SecurityException("회원 정보를 찾을 수 없습니다."));
//        }
